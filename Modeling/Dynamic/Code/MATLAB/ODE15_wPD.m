...
M0 = 800;  % Nm magnitude of static moment
...
plot(x_nodes, 1e3*w_static, '-','LineWidth',1,'MarkerFaceColor','r');
xlabel('beam length (m)', 'FontName', 'Times New Roman', 'FontSize', 11);
ylabel('vertical deflection (mm)', 'FontName', 'Times New Roman', 'FontSize', 11);
%title('static deflection under moment couple', 'FontName', 'Times New Roman', 'FontSize', 11);
grid on; box on;
...
xlabel('beam length (m)', 'FontName', 'Times New Roman', 'FontSize', 11);
ylabel('vertical deflection (mm)', 'FontName', 'Times New Roman', 'FontSize', 11);
%title('static deflection under reversed moment couple', 'FontName', 'Times New Roman', 'FontSize', 11);
grid on; box on;
...
dt = 1e-4; Tmax = 1.5; Nt = round(Tmax/dt); t = (0:Nt)*dt;
...
impulse_start_time = 0.01;  % seconds
...
% Settling threshold (customizable)
settle_thresh_free = 0.05 * peak_free;
settle_thresh_pd   = 0.05 * peak_pd;

% Number of steps for sustained settling (e.g. 5 ms)
sustain_steps = round(0.005 / dt);  % 5 ms window

% Start looking AFTER the peak time
...
% Check from peak onward for sustained below-threshold displacement
...
fprintf('  Peak Displacement (mm): Free = %.2f | PD = %.2f\n', 1e3 * peak_free, 1e3 * peak_pd);

fprintf('  Settling Time (ms):     ');
if ~isnan(settle_free_ms)
    fprintf('Free = %.2f | ', settle_free_ms);
else
    fprintf('Free = --    | ');
end
if ~isnan(settle_pd_ms)
    fprintf('PD = %.2f\n', settle_pd_ms);
else
    fprintf('PD = --\n');
end

fprintf('  RMS Accel (m/s^2):      Free = %.2f | PD = %.2f\n', rms_a_free, rms_a_pd);
%% ------------------------------------------------------------------------
