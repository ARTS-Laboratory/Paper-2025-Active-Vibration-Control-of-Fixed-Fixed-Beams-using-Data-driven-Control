...
# Redefine necessary reduced matrices and parameters from previous execution
# (For clean context)
nElem = 50
E = 210e9
A = 0.01
...
def eom(t, y):
    ...
    a = np.linalg.solve(M_red, f - C_red @ v - K_red @ u)
...
sol = solve_ivp(eom, t_span, y0, t_eval=t_eval, method='RK45')

# Plot midpoint vertical displacement
plt.figure()
plt.plot(sol.t, sol.y[impulse_dof_local,:], linewidth=2)
plt.xlabel('Time (s)')
plt.ylabel('Vertical displacement at midpoint (m)')
plt.title('Dynamic response to impulse (ODE solver)')
plt.grid(True)
plt.show()
